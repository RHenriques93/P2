...
<?php
  ...
  $dados = $db->query("SELECT subarea.nome, servico. descricao FROM servico JOIN utilizador ON servico.id_utilizador = utilizador.id_utilizador JOIN subarea ON servico.id_subarea = subarea.id_subarea JOIN area ON subarea.id_area = area.id_area WHERE utilizador.id_utilizador = $id");
                                                                
  foreach($dados as $row) {
    echo'<li class="list-group-item text-dark">'.$row["nome"].'</li>
    <li class="list-group-item text-dark">'.$row["descricao"].'</li>';
  }
?>
</ul>
</div>
